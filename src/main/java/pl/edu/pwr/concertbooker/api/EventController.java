...
public class EventController {
    ...
    @PostMapping
    public ResponseEntity<CreateEventDto> createEvent(@RequestBody @Valid CreateEventDto createEventDto) {
        eventService.createEvent(createEventDto);
        return ResponseEntity.ok(createEventDto);
    }

    @PutMapping
    public ResponseEntity<UpdateEventDto> updateEvent(@RequestBody @Valid UpdateEventDto updateEventDto) throws EntityNotFoundException {
        eventService.updateEvent(updateEventDto);
        return ResponseEntity.accepted().body(updateEventDto);
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<String> cancelEvent(@PathVariable long id) throws EntityNotFoundException {
